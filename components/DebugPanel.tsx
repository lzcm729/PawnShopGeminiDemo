...
export const DebugPanel: React.FC = () => {
  ...
  const { elementRef, handleMouseDown, style: dragStyle, isDragging } = useDraggable({
    initialPosition: { x: 16, y: window.innerHeight - PANEL_HEIGHT - 16 },
    storageKey: DEBUG_PANEL_POSITION_KEY,
    ...
    <>
      ...
      {state.showDebug && (
        ...
        >
            ...
            <div
              className={`flex justify-between items-center p-2 border-b border-green-900 bg-[#0a0a0a] select-none ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
