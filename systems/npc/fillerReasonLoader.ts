...
import reasonsCsv from '@/assets/data/texts/filler_reasons.csv?raw';
...
/**
 * Initialize reason templates from statically imported CSV data.
 * Optional override can be passed for testing.
 */
export function initializeFillerReasons(csvContent?: string): void {
  if (initialized && !csvContent) return;

  loadReasonsFromCSV(csvContent || reasonsCsv);
  initialized = true;
}
