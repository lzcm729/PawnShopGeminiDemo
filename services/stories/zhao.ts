...
export const ZHAO_CHAIN_INIT: EventChainState = {
    ...
    variables: { 
        ...
        trust: 50, 
        stress: 0,
        medal_extended: 0, // Track if medal was extended in Node 3
        day: 0
    },
    simulationRules: [
        { type: 'DELTA', targetVar: 'day', value: 1 },
        { type: 'DELTA', targetVar: 'funds', value: -500 }, // Daily living cost
        // Funding Injections to ensure he can afford specific plot points if player didn't fleece him too hard
        ...
        // End Game Mail Trigger (Virtual Event logic simulated via check)
        // Note: Real ending is triggered by Node 6 event, this is backup or flavor
    ]
};

export const ZHAO_EVENTS: StoryEvent[] = [
    // --- NODE 1: Day 1 [Pawn] Medal ---
    {
        id: "zhao_01_medal",
        chainId: "chain_zhao",
    ...
    // --- NODE S1-1: Day 4 [Deriv] Collector Lowball ---
    {
        ...
        template: {
            ...
            interactionType: 'NEGOTIATION', // Buying
        ...
        outcomes: {
            // Player SELLS to collector -> Medal becomes SOLD (Breach)
            "deal_standard": [
                { type: "ADD_FUNDS", value: 5000 }, 
                { type: "FORCE_SELL_TARGET", templateId: "zhao_item_medal" }, // Use ID reference logic or just targetItemId if matched
                { type: "SET_STAGE", value: 2 }, 
                { type: "MODIFY_REP", value: -10 }
            ]
        },
        targetItemId: "zhao_item_medal", // Used by FORCE_SELL_TARGET
        onReject: [{ type: "SET_STAGE", value: 2 }] 
    },

    // --- NODE 2: Day 6 [Pawn] Certificate ---
    {
        id: "zhao_03_cert",
        chainId: "chain_zhao",
        ...
        outcomes: {
            // All outcomes lead to Stage 3 and schedule the Collector Mail
    ...
    // --- NODE 3: Day 8 [Redemption] Medal Expiry ---
    {
        ...
        // If extend/fail, we move to stage 4 anyway so S1-2 can trigger (Collector Highball)
        ...
        template: {
            ...
            // REMOVED: redemptionIntent: 'REDEEM', so it can fallback to EXTEND based on funds
            dialogue: {
    ...
    // --- NODE S1-2: Day 8 [Deriv] Collector Highball ---
    {
        id: "zhao_05_collector_high",
        chainId: "chain_zhao",
        // Trigger: Stage 4 (After Node 3), Day >= 8
        triggerConditions: [{ variable: "stage", operator: "==", value: 4 }, { variable: "day", operator: ">=", value: 8 }],
        // This is a virtual deal, no item exchange from player side, just money and narrative
        ...
        outcomes: {
            ...
            "deal_standard": [
                ...
                { type: "FORCE_SELL_ALL" }, 
                { type: "SET_STAGE", value: 99 }, // End chain
                { type: "MODIFY_REP", value: -20 }, // Huge Rep Hit
    ...
    // --- NODE 4: Day 12 [Redemption] Final ---
    {
        id: "zhao_06_final_redemption",
        chainId: "chain_zhao",
        type: "REDEMPTION_CHECK",
        triggerConditions: [{ variable: "stage", operator: "==", value: 5 }, { variable: "day", operator: ">=", value: 12 }],
        targetItemId: "zhao_item_cert", // Core item to check is the certificate now, as medal might have been redeemed or extended
        // If medal was redeemed in Node 3, he comes for the Cert. If Medal was extended, he comes for BOTH.
        failureMailId: "mail_zhao_plea",
